...
required=(POSTGRES_DSN CODESTRA_AI_BACKUP_DIR CODESTRA_AI_RESTORE_EVIDENCE_DIR)
...
constraint_count=$(psql "$POSTGRES_DSN" -XAtq -v ON_ERROR_STOP=1 <<'SQL'
select count(*) from pg_indexes
where schemaname = 'public' and tablename = 'ai_requests'
  and indexname = 'uq_ai_request_idempotency';
SQL
)
