...
[[ "${ALLOW_ISOLATED_RESTORE:-false}" == "true" ]] || {
  echo "isolated restore requires explicit authorization" >&2
  exit 2
}
required=(PGHOST PGPORT PGDATABASE PGUSER PGPASSFILE CODESTRA_AI_BACKUP_DIR CODESTRA_AI_RESTORE_EVIDENCE_DIR CODESTRA_AI_RECOVERY_WORK_ROOT CODESTRA_EXPECTED_RELEASE_SHA CODESTRA_EXPECTED_IMAGE_DIGEST CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT)
for name in "${required[@]}"; do
  [[ -n "${!name:-}" ]] || { echo "required recovery setting is missing: $name" >&2; exit 2; }
done

backup_dir=$CODESTRA_AI_BACKUP_DIR
evidence_root=$CODESTRA_AI_RESTORE_EVIDENCE_DIR
...
[[ ! -L "$CODESTRA_AI_RECOVERY_WORK_ROOT" && -d "$CODESTRA_AI_RECOVERY_WORK_ROOT" ]] || { echo "recovery work root is invalid" >&2; exit 2; }
[[ "$(findmnt -n -o FSTYPE -T "$CODESTRA_AI_RECOVERY_WORK_ROOT")" == tmpfs ]] || { echo "plaintext recovery work requires tmpfs" >&2; exit 2; }
...
[[ "$(metadata_value SCHEMA)" == "codestra-ai-backup.v1" ]] || { echo "unsupported backup schema" >&2; exit 2; }
...
work=$(mktemp -d "$CODESTRA_AI_RECOVERY_WORK_ROOT/restore.XXXXXX")
...
[[ "$table_count" == "1" ]] || { echo "required table verification failed" >&2; exit 1; }
column_count=$(psql -XAtq -v ON_ERROR_STOP=1 <<'SQL'
select count(*) from information_schema.columns
where table_schema = 'public' and table_name = 'ai_requests'
  and column_name in ('tenant_id','idempotency_key','request_fingerprint','status');
SQL
)
[[ "$column_count" == "4" ]] || { echo "required column verification failed" >&2; exit 1; }
constraint_count=$(psql -XAtq -v ON_ERROR_STOP=1 <<'SQL'
select count(*)
from pg_index i
join pg_class idx on idx.oid=i.indexrelid
join pg_class tbl on tbl.oid=i.indrelid
join pg_namespace ns on ns.oid=tbl.relnamespace
where ns.nspname='public' and tbl.relname='ai_requests'
  and idx.relname='uq_ai_request_idempotency'
...
[[ "$constraint_count" == "1" ]] || { echo "idempotency constraint verification failed" >&2; exit 1; }
...
cat >"$result" <<EOF
SCHEMA=codestra-ai-restore-result.v1
STAMP=$stamp
