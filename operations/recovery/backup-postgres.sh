...
required=(PGHOST PGPORT PGDATABASE PGUSER PGPASSFILE CODESTRA_AI_BACKUP_ROOT CODESTRA_AI_RECOVERY_WORK_ROOT CODESTRA_RELEASE_SHA CODESTRA_IMAGE_DIGEST CODESTRA_BACKUP_GPG_RECIPIENT CODESTRA_BACKUP_GPG_SIGNING_FINGERPRINT)
...
backup_root=$CODESTRA_AI_BACKUP_ROOT
...
work=$(mktemp -d "$CODESTRA_AI_RECOVERY_WORK_ROOT/${stamp}.XXXXXX")
...
[[ "$database_name" == "codestra_ai" || "$database_name" == "codestra_ai_stage5" ]] || {
